...
public class CategoryArgumentType implements ArgumentType<String> {
   ...
   public String parse(StringReader reader) throws CommandSyntaxException {
      ...
      if (!EXAMPLES.contains(cat)) {
         throw (new DynamicCommandExceptionType((name) -> {
            return class_2561.method_43470(ClientSettings.isRu() ? "Категории " + name.toString() + " не существует(" : "Category " + name.toString() + " does not exist :(");
         })).create(cat);
      } else {
         return cat;
      }
   ...
   public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
      return class_2172.method_9265(EXAMPLES, builder);
   }
   ...
   static {
      EXAMPLES = Managers.MODULE.getCategories().stream().map(Module.Category::getName).toList();
   }
}
